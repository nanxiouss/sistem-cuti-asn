...
class RiwayatController extends Controller
{
    ...
    {
        // 1. Base Query: Ambil pengajuan lintas bidang yang sudah pernah diproses oleh Kasumum
        $query = Pengajuan::with(['user.pegawai.bidang', 'jenisCuti'])
            ->where(function ($q) {
                $q->whereNotNull('tgl_ttd_kasubbag_umum') // Sudah pernah disetujui/ditandatangani Kasumum
                  ->orWhereIn('status', ['Disetujui', 'Selesai', 'disetujui', 'selesai']) // Otomatis muncul jika status akhirnya sudah final
                  ->orWhere(function ($sub) {
                      // Jika ditolak, pastikan sudah lolos dari Kabid (artinya berkasnya sempat masuk ke Kasumum)
                      $sub->whereIn('status', ['Ditolak', 'ditolak'])
                          ->whereNotNull('tgl_ttd_kabid'); 
                  });
            });

        // 2. Filter Berdasarkan Status Fleksibel & Kebal Bug Huruf Kapital
        if ($request->filled('status')) {
            $status = $request->status;
            
            if (in_array(strtolower($status), ['diproses', 'proses'])) {
                // Berkas masih berjalan di tingkat atas (Sekdin/Kadin), kita kecualikan semua status final
        ...
        return view('kasumum.riwayat.index', compact('riwayatPengajuan', 'daftarTahun'));
    }
