<x-layouts.kadin.app>
    ...
    @php
        ...
        $pengajuanPegawai = $pengajuan->filter(function($item) {
            return !in_array($item->user->role ?? '', ['kasi', 'kasubbag_umum']);
        });

        $pengajuanStruktural = $pengajuan->filter(function($item) {
            return in_array($item->user->role ?? '', ['kasi', 'kasubbag_umum']);
        });
    @endphp
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                    <h3 class="font-bold text-slate-800 text-base flex items-center gap-2.5">
                        ...
                        Antrean Berkas Pegawai <span class="px-2 py-0.5 bg-indigo-100 text-indigo-700 text-xs rounded-md font-extrabold">{{ $pengajuanPegawai->count() }}</span>
                    </h3>
                </div>
                        ...
                        <tbody class="divide-y divide-slate-100 text-slate-700">
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-slate-400">
                                    <p class="font-medium text-slate-500 text-sm">Tidak ada antrean berkas dari Pegawai.</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
            ...
            {{-- ====================== BAGIAN BAWAH: ANTREAN KASI & KASUBBAG UMUM ====================== --}}
            <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                    <h3 class="font-bold text-slate-800 text-base flex items-center gap-2.5">
                        <i class="fas fa-user-shield text-amber-600 text-lg"></i>
                        Antrean Berkas Pejabat Struktural <span class="px-2 py-0.5 bg-amber-100 text-amber-800 text-xs rounded-md font-extrabold">{{ $pengajuanStruktural->count() }}</span>
                    </h3>
                </div>
                ...
                <div class="overflow-x-auto hide-scrollbar">
                    <table class="w-full text-left text-sm whitespace-nowrap border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 text-slate-700">
                            @forelse($pengajuanStruktural as $item)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="font-bold text-slate-900">{{ $item->user->nama ?? '-' }}</div>
                                    <div class="text-[10px] text-amber-700 font-bold uppercase tracking-wide bg-amber-50 px-2 py-0.5 rounded-md inline-block mt-1">{{ $item->user->pegawai->jabatan ?? '-' }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-slate-100 text-slate-700 rounded-lg text-xs font-semibold border border-slate-200/60">
                                        {{ $item->user->pegawai->bidang->nama_bidang ?? 'Global' }}
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-slate-400">
                                    <p class="font-medium text-slate-500 text-sm">Tidak ada antrean berkas dari Pejabat Struktural (Kasi / Kasubbag Umum).</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
