<x-layouts.admin.app>
    <div class="mb-6 flex items-center justify-between no-print">
        <div>
            ...
            <p class="text-slate-500 text-sm">Formulir BKN otomatis selesai di-generate. Rilis dokumen agar dapat
                dicetak oleh pegawai.</p>
        </div>
    ...
    <div
        class="mb-6 p-6 bg-white rounded-2xl border border-amber-200 shadow-sm bg-gradient-to-r from-amber-50/40 to-transparent no-print">
        <h3 class="font-bold text-amber-800 text-base mb-2 flex items-center gap-2">
            Finalisasi & Rilis Dokumen
        </h3>
        <p class="text-sm text-slate-600 mb-4">
            Seluruh draf penilaian birokrasi dan tanda tangan kedinasan telah terisi otomatis di bawah. Klik tombol di
            bawah untuk merilis dokumen ini ke akun pegawai dan mengubah status menjadi <strong>Selesai</strong>.
        </p>

        <form action="{{ route('admin.pemberkasan.proses', $pengajuan->id) }}" method="POST">
            @csrf
            <button type="submit"
                class="px-6 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-200 transition text-sm flex items-center gap-2">
    ...
    @else
    <div
        class="mb-6 p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl flex items-center justify-between no-print">
        <div class="text-sm">
            <strong>Status Selesai:</strong> Surat cuti ini telah difinalisasi dan dirilis. Pegawai saat ini sudah dapat
            mencetak dokumen ini.
        </div>
        <button onclick="window.print()"
            class="px-5 py-2.5 bg-slate-900 text-white text-sm font-bold rounded-lg hover:bg-slate-800 transition flex items-center gap-1.5 shadow-md">
            Cetak Formulir Cuti
        </button>
    </div>
    @endif
    ...
    @php
    // Identifikasi Role Pemohon
    $rolePemohon = $pengajuan->user->role ?? '';
    $isKasi = $rolePemohon === 'kasi' || str_contains(strtolower($pengajuan->user->pegawai->jabatan ?? ''), 'kasi');
    $isKasubbag = $rolePemohon === 'kasubbag_umum' || $rolePemohon === 'kasubbag';
    @endphp
    ...
    {{-- ========================================================================= --}}
    <div
        class="bg-white p-6 md:p-8 shadow-sm border border-slate-200 mx-auto print:shadow-none print:border-none print:p-0 font-sans text-black leading-snug max-w-[800px] antialiased text-[11px]">
        ...
        <div class="flex items-center justify-center border-b-[3px] border-black pb-2 mb-2">
            <div class="w-16">
                ...
                <img src="{{ asset('images/logosumsel.png') }}" alt="Logo Sumsel"
                    class="w-14 h-auto opacity-80 mix-blend-multiply">
            </div>
        ...
        <table class="w-full border-collapse border border-black mb-1">
            <tr>
                <td colspan="4" class="border border-black px-1.5 py-0.5 bg-slate-100 font-bold text-[10px]">I. DATA
                    PEGAWAI</td>
            </tr>
            <tr class="text-[10px]">
                ...
                <td class="border border-black px-1.5 py-0.5 w-[35%]">{{ $pengajuan->user->pegawai->nip ??
                    $pengajuan->user->nip ?? '-' }}</td>
            </tr>
            <tr class="text-[10px]">
                <td class="border border-black px-1.5 py-0.5">Jabatan</td>
                <td class="border border-black px-1.5 py-0.5">{{ $pengajuan->jabatan_pegawai ??
                    $pengajuan->user->pegawai->jabatan ?? '-' }}</td>
            ...
            <tr class="text-[10px]">
                ...
                <td colspan="3" class="border border-black px-1.5 py-0.5">Dinas Energi dan Sumber Daya Mineral Prov.
                    Sumsel</td>
            </tr>
        </table>
        ...
        <table class="w-full border-collapse border border-black mb-1 text-[10px]">
            <tr>
                <td colspan="4" class="border border-black px-1.5 py-0.5 bg-slate-100 font-bold">II. JENIS CUTI YANG
                    DIAMBIL **</td>
            </tr>
            <tr>
                <td class="border border-black px-1.5 py-0.5 w-[40%]">1. Cuti Tahunan</td>
                <td class="border border-black px-1.5 py-0.5 text-center w-[10%] font-bold">{{ $pengajuan->jenis_cuti_id
                    == 1 ? '√' : '' }}</td>
            ...
            <tr>
                ...
                <td class="border border-black px-1.5 py-0.5 text-center font-bold">{{ $pengajuan->jenis_cuti_id == 2 ?
                    '√' : '' }}</td>
                <td class="border border-black px-1.5 py-0.5">5. Cuti Alasan Penting</td>
                <td class="border border-black px-1.5 py-0.5 text-center font-bold">{{ $pengajuan->jenis_cuti_id == 5 ?
                    '√' : '' }}</td>
            </tr>
            <tr>
                <td class="border border-black px-1.5 py-0.5">3. Cuti Sakit</td>
                <td class="border border-black px-1.5 py-0.5 text-center font-bold">{{ $pengajuan->jenis_cuti_id == 3 ?
                    '√' : '' }}</td>
                <td class="border border-black px-1.5 py-0.5">6. Cuti di Luar Tanggungan Negara</td>
                <td class="border border-black px-1.5 py-0.5 text-center font-bold">{{ $pengajuan->jenis_cuti_id == 6 ?
                    '√' : '' }}</td>
            </tr>
        </table>
        ...
        <table class="w-full border-collapse border border-black mb-1 text-[10px]">
            <tr>
                <td colspan="5" class="border border-black px-1.5 py-0.5 bg-slate-100 font-bold">V. CATATAN CUTI ***
                </td>
            </tr>
            <tr>
                <td colspan="3" class="border border-black px-1.5 py-0.5 w-[45%]">1. CUTI TAHUNAN {{
            ...
            <tr class="text-center">
                ...
                <td class="border border-black px-1.5 py-0.5 font-bold">{{ $pengajuan->jenis_cuti_id == 3 ? '√' : '' }}
                </td>
            </tr>
            <tr class="text-center">
                ...
                <td class="border border-black px-1.5 py-0.5 font-bold">{{ $pengajuan->jenis_cuti_id == 4 ? '√' : '' }}
                </td>
            </tr>
            <tr class="text-center">
                ...
                <td class="border border-black px-1.5 py-0.5 font-bold">{{ $pengajuan->jenis_cuti_id == 5 ? '√' : '' }}
                </td>
            </tr>
            <tr class="text-center">
                ...
                <td class="border border-black px-1.5 py-0.5 font-bold">{{ $pengajuan->jenis_cuti_id == 6 ? '√' : '' }}
                </td>
            </tr>
        </table>
        ...
        <table class="w-full border-collapse border border-black mb-1 table-fixed text-[10px]">
            <tr>
                <td colspan="3" class="border border-black px-1.5 py-0.5 bg-slate-100 font-bold">VI. ALAMAT SELAMA
                    MENJALANKAN CUTI</td>
            </tr>
            <tr>
            ...
            <tr class="h-[65px]">
                ...
                @if($isKasubbag)
                <td colspan="2" class="border border-black px-1.5 py-0.5 align-top text-center w-[70%]">
                ...
                <td colspan="2" class="border border-black px-2 py-1 align-top text-justify w-[35%] relative">
                    <p class="text-[9px] italic text-slate-700 underline mb-1">Catatan Kabid {{ $pengajuan->bidang_kabid
                        ?? ($kabid->pegawai->bidang->nama_bidang ?? '') }} :</p>
                ...
                <td class="border border-black px-1.5 py-0.5 align-top text-justify w-[35%] relative">
                    <p class="text-[9px] italic text-slate-700 underline mb-0.5">Catatan Kasi {{ $pengajuan->bidang_kasi
                        ?? ($pengajuan->atasan->pegawai->bidang->nama_bidang ?? '') }} :</p>
                ...
                <td class="border border-black px-1.5 py-0.5 align-top text-justify relative w-[30%]">
                    ...
                    <div class="h-14 flex justify-normal mt-6 mb-2  mx-2 relative">
                        @if($pengajuan->ttd_pegawai)
                        <img src="{{ asset('storage/' . $pengajuan->ttd_pegawai) }}"
                            class="h-14 object-contain mix-blend-multiply" alt="TTD Pegawai">
                        @else
                    ...
                    <p class="text-[9px] mx-1">NIP. {{ $pengajuan->user->pegawai->nip ?? $pengajuan->user->nip ?? '-' }}
                    </p>
                </td>
            </tr>
        </table>
        ...
        <table class="w-full border-collapse border border-black mb-1 table-fixed text-[10px]">
            <tr>
                <td colspan="4" class="border border-black px-1.5 py-0.5 bg-slate-100 font-bold">VII. PERTIMBANGAN
                    ATASAN LANGSUNG **</td>
            </tr>
            ...
            <tr class="h-[70px]">
                <td colspan="3" class="border border-black px-2 py-1 align-top relative">
                    @if($isKasubbag)
                    <span class="text-slate-400 italic mt-6 flex justify-center">-</span>
                    @else
                    <p class="text-[9px] italic text-slate-700 underline mb-1">Catatan Kasubbag. Umkep:</p>
                ...
                <td class="border border-black px-1.5 py-0.5 align-top text-justify relative">
                    @if($isKasubbag)
                    <p class="text-left text-[9px] my-1 mx-1">Sekretaris Dinas,</p>
                    <div class="h-14 flex justify-normal my-1 mx-1 relative">
                        @if($pengajuan->ttd_sekdin)
                        <img src="{{ asset('storage/' . $pengajuan->ttd_sekdin) }}"
                            class="h-14 object-contain mix-blend-multiply" alt="TTD Sekdin">
                        @else
                        <span class="text-[9px] text-slate-400 italic">[Belum TTD]</span>
                        @endif
                    </div>
                    <p class="font-bold underline uppercase mx-1 text-[10px]">{{ $pengajuan->nama_sekdin ??
                        '.......................' }}</p>
                    <p class="text-[9px] flex justify-normal mx-1">NIP. {{ $pengajuan->nip_kadin ?? ($kadin->nip ?? '-')
                        }}</p>
                    @elseif($isKasi)
                    <p class="text-left text-[9px] my-1 mx-1">Kabid {{ $pengajuan->jabatan_kabid ?? 'Kabid' }},</p>
                    <div class="h-14 flex justify-normal my-1 mx-1 relative">
                        @if($pengajuan->ttd_kabid)
                        <img src="{{ asset('storage/' . $pengajuan->ttd_kabid) }}"
                            class="h-14 object-contain mix-blend-multiply" alt="TTD Kabid">
                        @else
                        <span class="text-[9px] text-slate-400 italic">[Belum TTD]</span>
                        @endif
                    </div>
                    <p class="font-bold underline uppercase mx-1 text-[10px]">{{ $pengajuan->nama_kabid ?? ($kabid->nama
                        ?? ($pengajuan->atasan->nama ?? '-')) }}</p>
                    <p class="text-[9px] mx-1">NIP. {{ $pengajuan->nip_kabid ?? ($kabid->nip ?? ($pengajuan->atasan->nip
                        ?? '-')) }}</p>
                    @else
                    <p class="text-left text-[9px] my-1 mx-1">Kasi {{ $pengajuan->jabatan_kasi ?? 'Kasi' }},</p>
                    <div class="h-14 flex justify-normal my-1 mx-1 relative">
                        @if($pengajuan->ttd_kasi)
                        <img src="{{ asset('storage/' . $pengajuan->ttd_kasi) }}"
                            class="h-14 object-contain mix-blend-multiply" alt="TTD Kasi">
                        @else
                        <span class="text-[9px] text-slate-400 italic">[Belum TTD]</span>
                        @endif
                    </div>
                    <p class="font-bold underline uppercase mx-1 text-[10px]">{{ $pengajuan->nama_kasi ??
                        ($pengajuan->atasan->nama ?? '-') }}</p>
                    <p class="text-[9px] mx-1">NIP. {{ $pengajuan->nip_kasi ?? ($pengajuan->atasan->nip ?? '-') }}</p>
                    @endif
                </td>
            </tr>
        </table>
        ...
        <table class="w-full border-collapse border border-black mb-1 table-fixed text-[10px]">
            <tr>
                <td colspan="4" class="border border-black px-1.5 py-0.5 bg-slate-100 font-bold">VIII. KEPUTUSAN PEJABAT
                ...
                <td class="border border-black px-1.5 py-0.5 align-top justify-normal relative">
                    ...
                    <div class="h-14 flex justify-normal my-1 mx-1 relative">
                        ...
                        <img src="{{ asset('storage/' . $pengajuan->ttd_kadin) }}"
                            class="h-14 object-contain mix-blend-multiply opacity-95" alt="TTD Kadin">
                        @else
                        <span class="text-[9px] text-slate-400 italic">[Belum TTD Kadin]</span>
                        @endif
                    </div>
                    <p class="font-bold flex justify-normal mx-1 underline uppercase text-[10px]">{{
                        $pengajuan->nama_kadin ?? ($kadin->nama ?? '-') }}</p>
                    <p class="text-[9px] flex justify-normal mx-1">NIP. {{ $pengajuan->nip_kadin ?? ($kadin->nip ?? '-')
                        }}</p>
                </td>
            </tr>
