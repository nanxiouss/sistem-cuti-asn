<x-layouts.kasumum.app>
    ...
    @php
        $pengajuanPegawai = $pengajuan->filter(fn($item) => ($item->user->role ?? '') !== 'kasi');
        $pengajuanKasi = $pengajuan->filter(fn($item) => ($item->user->role ?? '') === 'kasi');
    @endphp
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            {{-- ====================== BAGIAN BAWAH: ANTREAN KASI ====================== --}}
            <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                    <h3 class="font-bold text-slate-800 text-base flex items-center gap-2.5">
                        <i class="fas fa-user-shield text-lime-600 text-lg"></i>
                        Antrean Berkas Kepala Seksi (Kasi) <span class="px-2 py-0.5 bg-lime-100 text-lime-800 text-xs rounded-md font-extrabold">{{ $pengajuanKasi->count() }}</span>
                    </h3>
                </div>
                ...
                <div class="overflow-x-auto hide-scrollbar">
                    <table class="w-full text-left text-sm whitespace-nowrap border-collapse">
                        <thead class="bg-slate-50/70 text-slate-500 font-bold uppercase text-xs tracking-wider border-b border-slate-100">
                            <tr>
                                <th class="px-6 py-4">Nama Kasi</th>
                                <th class="px-6 py-4">Asal Bidang</th>
                                <th class="px-6 py-4">Jenis Cuti</th>
                                <th class="px-6 py-4">Tanggal Pelaksanaan</th>
                                <th class="px-6 py-4 text-center">Durasi</th>
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center text-slate-400">
                                    <div class="flex flex-col items-center justify-center gap-2">
                                        <i class="fas fa-circle-check text-4xl text-slate-200"></i>
                                        <p class="font-medium text-slate-600 text-sm">Tidak ada antrean berkas dari Kasi.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
