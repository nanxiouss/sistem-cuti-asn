<x-layouts.kasumum.app>
    <x-slot:title>Formulir Pengajuan Cuti (Kasubagg Umum dan Kepegawaian)</x-slot:title>
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="mb-8 text-center md:text-left">
            ...
            <p class="text-slate-500 mt-1">Lengkapi formulir sesuai data yang benar untuk diproses oleh Kepala Bidang.</p>
        </div>

        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="p-8">
                {{-- PERUBAHAN 2: Route action form diarahkan ke route store milik kasi --}}
                <form action="{{ route('kasumum.pengajuan.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="mb-8 border-t border-slate-200 pt-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-slate-500 mb-1">Bidang / Seksi</label>
                        ...
                        <div class="relative parent-dropdown">
                            ...
                            <button type="button" @if($belumSatuTahun) disabled @endif onclick="toggleAtasanDropdown(event)" class="w-full inline-flex items-center justify-between text-slate-700 bg-white border border-slate-300 {{ $belumSatuTahun ? 'bg-slate-100 cursor-not-allowed opacity-70' : 'hover:bg-slate-50' }} font-medium rounded-lg text-sm px-4 py-3 transition shadow-sm focus:ring-2 focus:ring-amber-300">
                                ...
                                <span id="label_id_atasan" class="truncate pr-4">
                                    ...
                                    @else
                                    -- Pilih Kabid yang akan memverifikasi berkas Anda --
                                    @endif
                                </span>
                                <svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 9-7 7-7-7" />
                            ...
                            <div id="dropdown_id_atasan" class="hidden absolute z-10 w-full bg-white border border-slate-200 rounded-lg shadow-xl mt-1 max-h-64 overflow-y-auto border-t-4 border-t-amber-400">
                                <ul class="p-2 text-sm text-slate-700">
                                    {{-- Data atasans ini dikirim dari Kasi/PengajuanController (Sudah di-filter hanya Kabid) --}}
                    ...
                    <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
                        {{-- PERUBAHAN 3: Route Batal diarahkan ke dashboard Kasi --}}
                        <a href="{{ route('kasi.dashboard') }}" class="px-6 py-3 bg-white text-slate-700 font-bold rounded-xl border border-slate-300 hover:bg-slate-50 transition">Batalkan</a>
