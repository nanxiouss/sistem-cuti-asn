    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-0.5">Monitoring & persetujuan berkas cuti tingkat Sekretariat / Dinas.</p>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex items-center justify-between transition-all hover:shadow-md">
                    <div class="space-y-1">
                        <span class="text-xs font-bold uppercase tracking-wider text-slate-400 block">Antrian Persetujuan</span>
                        ...
                        <p class="text-[11px] text-amber-600 font-medium flex items-center gap-1 pt-1">
                            <i class="fas fa-clock"></i> Menunggu acc Sekdin
                        </p>
                    </div>
                ...
                <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50/50">
                    <div>
                        <h3 class="text-base font-bold text-slate-800 flex items-center gap-2">
                            <i class="fas fa-list text-amber-500"></i> Berkas Masuk Perlu Persetujuan
                        </h3>
                        <p class="text-xs text-slate-500 mt-0.5">Daftar pengajuan yang telah terverifikasi oleh Kasubbag Umum.</p>
                    </div>
                    <a href="{{ route('sekdin.persetujuan.index') }}" class="px-4 py-2 bg-slate-900 text-white hover:bg-slate-800 rounded-xl text-xs font-bold uppercase tracking-wider transition-all shadow-sm">
                        Lihat Semua Berkas
                    </a>
                </div>
                            ...
                            @forelse($pengajuanButuhAksi as $item)
                                <tr class="hover:bg-slate-50/80 transition-colors">
                                    ...
                                    <td class="py-4 px-6 text-center">
                                        <a href="{{ route('sekdin.persetujuan.show', $item->id) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-sky-400 hover:bg-sky-500 text-slate-900 font-extrabold rounded-xl text-xs uppercase tracking-wider transition-all shadow-sm shadow-sky-400/10">
                                ...
                                <tr>
                                    <td colspan="5" class="py-12 px-6 text-center">
                                        <div class="flex flex-col items-center justify-center gap-2 opacity-50">
                                            ...
                                            <p class="text-xs font-bold text-slate-500">Tidak ada berkas pengajuan baru yang perlu diproses.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
