<x-layouts.sekdin.app>
    <x-slot:title>Riwayat Proses Berkas - Sekretaris Dinas</x-slot:title>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                ...
                <p class="text-sm text-slate-500 mt-1">Daftar seluruh berkas pengajuan cuti yang telah Anda proses atau putuskan.</p>
            </div>

            {{-- Form Filter Card --}}
            <div class="bg-white rounded-3xl shadow-sm border border-slate-200 p-6 mb-6">
                <form action="{{ route('sekdin.riwayat.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full text-sm bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-slate-700 font-medium focus:outline-none focus:ring-2 focus:ring-slate-900">
                            ...
                            <option value="Diproses" {{ request('status') == 'Diproses' ? 'selected' : '' }}>Sedang Diproses (Menunggu Kadin)</option>
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('sekdin.riwayat.index') }}" class="px-4 py-3 bg-slate-200 hover:bg-slate-300 text-slate-700 font-medium rounded-xl transition-all flex items-center justify-center" title="Reset Filter">
                    ...
                    <table class="w-full text-left text-sm whitespace-nowrap border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 text-slate-700">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="{{ route('sekdin.riwayat.show', $item->id) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold rounded-xl transition-all border border-slate-200">
