                            ...
                            <div>
                                ...
                                <p class="font-medium text-slate-800">
                                    ...
                                    <span class="text-slate-400 font-normal">({{
                                        $pengajuan->user->pegawai->bidang->parent->nama_bidang }})</span>
                                    @else
                                    {{ $pengajuan->user->pegawai->bidang->nama_bidang ?? '-' }}
                                    @endif
                                </p>
                        ...
                        <div class="bg-slate-50 rounded-xl p-4 border border-slate-100 mb-5">
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <span class="px-3 py-1 bg-indigo-100 text-indigo-700 font-bold rounded-lg text-sm">
                                    {{ $pengajuan->jenisCuti->nama ?? $pengajuan->jenisCuti->nama_cuti ?? 'Cuti Tidak
                                    Diketahui' }}
                                </span>
                            </div>
                    ...
                    <div
                        ...
                        <div class="py-5 border-b border-slate-200">
                            ...
                            <div class="mt-4 ml-14 bg-slate-50 border border-slate-200 rounded-xl p-3">
                                <p class="text-xs font-bold text-slate-500 mb-1">
                                    <i class="fas fa-comment-alt mr-1"></i>
                                    Catatan Kasubbag Umum & Kepegawaian
                                </p>

                                <p class="text-sm text-slate-700 leading-relaxed break-words">
                                    {{ $pengajuan->catatan_kasubbag }}
                                </p>
                            </div>
                            @endif
                        </div>
                ...
                <div class="space-y-6">
                    <div class="bg-slate-900 rounded-2xl p-6 shadow-md text-white border border-slate-800">
                        <h3
                            ...
                            <i class="fas fa-pen-fancy text-lime-400"></i> Tindakan Sekdin
                        </h3>
                        <p class="text-xs text-slate-400 mb-5 leading-relaxed">
                            Berkas yang disetujui akan dibubuhkan spesimen TTD Anda dan diteruskan secara otomatis ke
                            meja Kepala Dinas (Kadin) untuk persetujuan akhir.
                        </p>
                        ...
                        <form action="{{ route('sekdin.persetujuan.update', $pengajuan->id) }}" method="POST">
                            ...
                            <div class="mb-4">
                                <label class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">
                                    Catatan Sekretaris Dinas <span
                                        class="text-[10px] text-rose-400 font-normal lowercase">(wajib jika
                                        menolak)</span>
                                </label>
                                <textarea name="catatan_sekdin" rows="3"
                                    class="w-full bg-slate-800 border-slate-700 text-white rounded-xl focus:ring-lime-500 focus:border-lime-500 text-xs placeholder-slate-500"
                                    placeholder="Tambahkan instruksi / alasan jika menolak..."></textarea>
                            </div>

                            <div class="mb-6 border-t border-slate-800 pt-4">
                                <label class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">
                                    Password Verifikasi <span
                                        class="text-[10px] text-lime-400 font-normal lowercase">(wajib jika
                                        setuju)</span>
                                </label>
                                <div class="relative">
                                    <span
                                        class="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-500 text-xs">
                                        <i class="fas fa-key"></i>
                                    </span>
                                    <input type="password" name="password_verifikasi"
                                        class="w-full bg-slate-800 border-slate-700 text-white rounded-xl pl-9 pr-4 py-2 focus:ring-lime-500 focus:border-lime-500 text-xs placeholder-slate-500"
                                        placeholder="Masukkan password login Anda...">
                                </div>
                                <p class="text-[10px] text-slate-500 mt-1 leading-normal">*Guna memberikan tanda tangan
                                    elektronik yang terdata pada sistem.</p>
                            </div>

                            <div class="flex flex-col gap-2.5">
                                <button type="submit" name="status" value="Disetujui"
                                    class="w-full flex items-center justify-center gap-2 py-3 bg-lime-400 hover:bg-lime-500 text-slate-900 font-extrabold rounded-xl text-xs uppercase tracking-wider transition-all shadow-lg shadow-lime-400/20">
                                    <i class="fas fa-check-circle text-sm"></i> SETUJUI & SEMATKAN TTD
                                </button>
                                <button type="submit" name="status" value="Ditolak"
                                    class="w-full flex items-center justify-center gap-2 py-3 bg-slate-800 hover:bg-rose-600 text-slate-400 hover:text-white font-bold rounded-xl text-xs uppercase tracking-wider transition-all border border-slate-700 hover:border-rose-600">
                                    <i class="fas fa-times-circle text-sm"></i> Tolak Berkas
                                </button>
                            </div>
                        </form>
                    </div>
