<nav x-data="{ open: false, profileOpen: false }" class="bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-50 transition-all duration-300 supports-[backdrop-filter]:bg-white/60">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div class="flex justify-between h-20">
            ...
            <div class="flex items-center gap-2 sm:gap-5">
                <a href="/" class="flex-shrink-0 flex items-center gap-3 sm:gap-4 group min-w-0">
                    ...
                    <div class="leading-tight border-l-2 border-slate-200 pl-3 sm:pl-4 truncate">
                        ...
                        <p class="text-[8px] sm:text-[9px] font-bold text-rose-500 uppercase tracking-[0.2em] truncate">PANEL Kepala Seksi</p>
                    </div>
                </a>
            </div>
            ...
            <div class="hidden md:flex items-center gap-1 bg-white/50 backdrop-blur-sm p-1 rounded-full border border-white/40 shadow-sm self-center">
                ...
                <a href="{{ route('kasi.dashboard') }}" class="{{ $navClass }} {{ request()->routeIs('kasi.dashboard') ? $activeClass : $inactiveClass }}">Dashboard</a>
                <a href="{{ route('kasi.persetujuan.index') }}" class="{{ $navClass }} {{ request()->routeIs('kasi.persetujuan*') ? $activeClass : $inactiveClass }}">Persetujuan Cuti</a>
                <a href="{{ route('kasi.pengajuan.create') }}" class="{{ $navClass }} {{ request()->routeIs('kasi.pengajuan*') ? $activeClass : $inactiveClass }}">Pengajuan Cuti</a>
                <a href="{{ route('kasi.riwayat.index') }}" class="{{ $navClass }} {{ request()->routeIs('kasi.riwayat*') ? $activeClass : $inactiveClass }}">Riwayat</a>
                <a href="{{ route('kasi.kalender.index') }}" class="{{ $navClass }} {{ request()->routeIs('kasi.kalender*') ? $activeClass : $inactiveClass }}">Kalender</a>
            </div>
                ...
                <div class="relative" @click.outside="profileOpen = false">
                    <button @click="profileOpen = !profileOpen" class="flex items-center gap-3 cursor-pointer group focus:outline-none p-1 rounded-full hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-200">
                        ...
                        <div class="hidden sm:flex flex-col items-start justify-center text-left pr-1">
                            ...
                            <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-rose-50 text-rose-600 tracking-wider uppercase border border-rose-100 mt-1">Kepala Seksi</span>
                        </div>
                    </button>
                    <!-- Dropdown Profile -->
    ...
    <div x-show="open" @click.away="open = false" class="md:hidden absolute top-full left-0 w-full bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-xl z-40" x-cloak>
        <div class="px-4 pt-2 pb-4 space-y-2">
            <a href="{{ route('kasi.dashboard') }}" class="block px-4 py-3 rounded-xl text-base font-medium text-slate-600 hover:bg-slate-50">Dashboard</a>
            <a href="{{ route('kasi.persetujuan.index') }}" class="block px-4 py-3 rounded-xl text-base font-medium text-slate-600 hover:bg-slate-50">Persetujuan Cuti</a>
            <a href="{{ route('kasi.pengajuan.create') }}" class="block px-4 py-3 rounded-xl text-base font-medium text-slate-600 hover:bg-slate-50">Pengajuan Cuti</a>
            <a href="{{ route('kasi.riwayat.index') }}" class="block px-4 py-3 rounded-xl text-base font-medium text-slate-600 hover:bg-slate-50">Riwayat</a>
            <a href="{{ route('kasi.kalender.index') }}" class="block px-4 py-3 rounded-xl text-base font-medium text-slate-600 hover:bg-slate-50">Kalender</a>
        </div>
    </div>
</nav>