<nav x-data="{ mobileMenu: false, profileOpen: false }" class="bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-50 transition-all duration-300 supports-[backdrop-filter]:bg-white/60">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div class="flex justify-between h-20">
            ...
            <div class="flex items-center gap-2 sm:gap-5">
                <a href="/" class="flex-shrink-0 flex items-center gap-3 sm:gap-4 group min-w-0">
                    ...
                    <div class="leading-tight border-l-2 border-slate-200 pl-3 sm:pl-4 truncate">
                        ...
                        <p class="text-[8px] sm:text-[9px] font-bold text-rose-500 uppercase tracking-[0.2em] truncate">PANEL Admin Kepegawaian</p>
                    </div>
                </a>
            </div>
            ...
            <div class="flex items-center gap-2 sm:gap-4 shrink-0">
                ...
                <!-- 1. Profile (Dipindah ke sebelah kiri hamburger) -->
                ...
                <!-- 2. Mobile Menu Button (Hamburger - Dipindah ke paling kanan) -->
                <div class="flex items-center md:hidden">
                    <button @click="mobileMenu = !mobileMenu" class="p-2 -mr-2 rounded-lg hover:bg-slate-100 transition focus:outline-none text-slate-600">
    ...
    <div x-show="mobileMenu" 
        ...
        <div class="px-4 pt-2 pb-4 space-y-2">
            ...
            @endphp
            <a href="{{ route('admin.dashboard') }}" class="{{ $mobileNavClass }} {{ request()->routeIs('admin.dashboard') ? $mobileActiveClass : $mobileInactiveClass }}">
                Dashboard
            </a>
            <a href="{{ route('admin.pegawai.index') }}" class="{{ $mobileNavClass }} {{ request()->routeIs('admin.pegawai*') ? $mobileActiveClass : $mobileInactiveClass }}">
                Kelola Pegawai
            </a>
            <a href="{{ route('admin.pengajuan.index') }}" class="{{ $mobileNavClass }} {{ request()->routeIs('admin.pengajuan*') ? $mobileActiveClass : $mobileInactiveClass }}">
                Data Pengajuan
            </a>
            <a href="{{ route('admin.pemberkasan.index') }}" class="{{ $mobileNavClass }} {{ request()->routeIs('admin.pemberkasan*') ? $mobileActiveClass : $mobileInactiveClass }}">
                Pemberkasan
            </a>
            <a href="{{ route('admin.kalender.index') }}" class="{{ $mobileNavClass }} {{ request()->routeIs('admin.kalender*') ? $mobileActiveClass : $mobileInactiveClass }}">
                Kalender
            </a>
            <a href="{{ route('admin.laporan.index') }}" class="{{ $mobileNavClass }} {{ request()->routeIs('admin.laporan*') ? $mobileActiveClass : $mobileInactiveClass }}">
                Laporan
            </a>
        </div>
    </div>
