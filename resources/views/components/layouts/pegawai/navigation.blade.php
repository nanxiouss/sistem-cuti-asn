<nav x-data="{ open: false, profileOpen: false }" class="bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-50 transition-all duration-300 supports-[backdrop-filter]:bg-white/60">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div class="flex justify-between h-20">
            ...
            <div class="flex items-center gap-2 sm:gap-5">
                <a href="/" class="flex-shrink-0 flex items-center gap-3 sm:gap-4 group min-w-0">
                    ...
                    <div class="leading-tight border-l-2 border-slate-200 pl-3 sm:pl-4 truncate">
                        ...
                        <p class="text-[8px] sm:text-[9px] font-bold text-slate-500 uppercase tracking-[0.2em] truncate">PANEL Pelaksana</p>
                    </div>
                </a>
            </div>
            ...
            <div class="hidden md:flex items-center gap-1 bg-white/50 backdrop-blur-sm p-1 rounded-full border border-white/40 shadow-sm self-center">
                ...
                <a href="{{ route('pegawai.dashboard') }}" class="{{ $navClass }} {{ request()->routeIs('pegawai.dashboard') ? $activeClass : $inactiveClass }}">Dashboard</a>
                <a href="{{ route('pegawai.pengajuan.create') }}" class="{{ $navClass }} {{ request()->routeIs('pegawai.pengajuan*') ? $activeClass : $inactiveClass }}">Ajukan Cuti</a>
                <a href="{{ route('pegawai.kalender.index') }}" class="{{ $navClass }} {{ request()->routeIs('pegawai.kalender*') ? $activeClass : $inactiveClass }}">Kalender</a>
                <a href="{{ route('pegawai.riwayat.index') }}" class="{{ $navClass }} {{ request()->routeIs('pegawai.riwayat*') ? $activeClass : $inactiveClass }}">Riwayat</a>
                <a href="{{ route('pegawai.regulasi.index') }}" class="{{ $navClass }} {{ request()->routeIs('pegawai.regulasi*') ? $activeClass : $inactiveClass }}">Regulasi</a>
            </div>
            ...
            <div class="flex items-center gap-2 sm:gap-4 shrink-0">
                ...
                <div class="relative" @click.outside="profileOpen = false">
                    <button @click="profileOpen = !profileOpen" class="flex items-center gap-3 cursor-pointer group focus:outline-none p-1 rounded-full hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-200">
                        <div class="w-9 h-9 rounded-full bg-gradient-to-tr from-lime-600 to-lime-600 p-[2px]">
                            <div class="w-full h-full rounded-full bg-white flex items-center justify-center overflow-hidden">
                                <span class="font-bold text-sm text-lime-600">{{ substr(Auth::user()->nama, 0, 1) }}</span>
                            </div>
                        </div>
                        <div class="hidden sm:flex flex-col items-start justify-center text-left pr-1">
                            <span class="text-sm font-bold text-slate-700 group-hover:text-lime-700 transition leading-tight">{{ Auth::user()->nama }}</span>
                            <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-lime-50 text-lime-600 tracking-wider uppercase border border-lime-100 mt-1">{{ Auth::user()->jabatan ?? 'Pelaksana' }}</span>
                        </div>
                    </button>
                    <!-- Dropdown Profile -->
                    <div x-show="profileOpen" x-transition class="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-200/60 z-50 overflow-hidden" x-cloak>
                        ...
                        <div class="p-2"><a href="/profile" class="block px-3 py-2 text-sm font-medium text-slate-600 rounded-xl hover:bg-slate-50 hover:text-lime-600">Edit Profile</a></div>
    ...
    <div x-show="open" @click.away="open = false" class="md:hidden absolute top-full left-0 w-full bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-xl z-40" x-cloak>
        <div class="px-4 pt-2 pb-4 space-y-2">
            <a href="{{ route('pegawai.dashboard') }}" class="block px-4 py-3 rounded-xl text-base font-medium text-slate-600 hover:bg-slate-50">Dashboard</a>
            <a href="{{ route('pegawai.pengajuan.create') }}" class="block px-4 py-3 rounded-xl text-base font-medium text-slate-600 hover:bg-slate-50">Ajukan Cuti</a>
            <a href="{{ route('pegawai.kalender.index') }}" class="block px-4 py-3 rounded-xl text-base font-medium text-slate-600 hover:bg-slate-50">Kalender</a>
            <a href="{{ route('pegawai.riwayat.index') }}" class="block px-4 py-3 rounded-xl text-base font-medium text-slate-600 hover:bg-slate-50">Riwayat</a>
            <a href="{{ route('pegawai.regulasi.index') }}" class="block px-4 py-3 rounded-xl text-base font-medium text-slate-600 hover:bg-slate-50">Regulasi</a>
        </div>
    </div>
</nav>